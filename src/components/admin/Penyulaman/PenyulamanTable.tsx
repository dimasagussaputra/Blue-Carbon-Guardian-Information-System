...
interface PenyulamanTableProps {
  onEdit: (record: PenyulamanRecord) => void;
  gelombang?: string;
}

function SurvivalBar({ rate }: { rate: number | null }) {
  if (rate === null) return <span className="text-sm text-slate-400 dark:text-slate-500">-</span>;
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{rate}%</span>
    </div>
  );
}

export default function PenyulamanTable({
  onEdit,
  gelombang,
}: PenyulamanTableProps) {
  const [data, setData] = useState<PenyulamanRecord[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const result: PenyulamanListResponse = await res.json();

      setData(result.data);
      setTotal(result.total);
      setTotalPages(result.totalPages);
    } catch {
      toast.error("Gagal memuat data penyulaman");
    } finally {
      setLoading(false);
    }
  }, [search, gelombang, statusFilter, sorting, page, limit]);
  ...
  useEffect(() => {
    setPage(1);
  }, [search, gelombang, statusFilter]);

  async function handleDelete(id: string) {
    setDeleting(true);
    try {
      const res = await fetch(`/api/penyulaman/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Gagal menghapus");
      setDeleteConfirm(null);
      fetchData();
      toast.success("Data penyulaman berhasil dihapus");
    } catch {
      toast.error("Gagal menghapus data penyulaman");
    } finally {
      setDeleting(false);
    }
  }

  const columns = useMemo<ColumnDef<PenyulamanRecord>[]>(
    () => [
      {
        accessorKey: "tanggal",
        header: "Tanggal",
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <span className="inline-flex items-center rounded-full bg-brand-blue-light/10 px-2.5 py-0.5 text-xs font-medium text-brand-blue-light">
              Gel. {val}
            </span>
          );
        },
      ...
      },
      {
        ...
        header: "Survival",
        cell: ({ row }) => (
          <SurvivalBar rate={row.original.survival_rate} />
        ),
      },
      {
        id: "foto",
        ...
        cell: ({ row }) => {
          ...
          return url ? (
            ...
            <span className="flex h-10 w-10 items-center justify-center rounded-lg border border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-300 dark:text-slate-500">
          ...
          return (
            <span
              className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${
                colors[val] ?? "bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300"
              }`}
            >
              {val}
            </span>
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            <button
              ...
              className="rounded-lg p-1.5 text-slate-400 dark:text-slate-500 transition hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-brand-green-medium"
              title="Edit"
            >
            ...
            <button
              ...
              className="rounded-lg p-1.5 text-slate-400 dark:text-slate-500 transition hover:bg-red-50 dark:hover:bg-slate-700/50 hover:text-red-500"
              title="Hapus"
            >
  ...
  return (
    <div className="rounded-2xl border border-slate-200/80 dark:border-slate-700/80 bg-white dark:bg-slate-800">
      <div className="flex flex-col gap-3 border-b border-slate-100 dark:border-slate-700 p-4 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
          <input
            type="text"
            placeholder="Cari spesies atau keterangan..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full rounded-xl border border-slate-200 dark:border-slate-700 py-2.5 pl-10 pr-3 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:border-brand-green-medium focus:outline-none focus:ring-2 focus:ring-brand-green-medium/20 dark:bg-slate-800"
          />
        </div>
        <div className="flex gap-2">
          <select
            ...
            className="rounded-xl border border-slate-200 dark:border-slate-700 px-3 py-2.5 text-sm text-slate-600 dark:text-slate-300 focus:border-brand-green-medium focus:outline-none focus:ring-2 focus:ring-brand-green-medium/20 dark:bg-slate-800"
          >
          ...
          <table className="w-full">
            ...
            <tbody>
              {table.getRowModel().rows.map((row) => (
                <tr
                  key={row.id}
                  className="border-b border-slate-50 dark:border-slate-700/50 transition hover:bg-slate-50/50 dark:hover:bg-slate-700/30"
                >
      ...
      <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-700 px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded-xl border border-slate-200 dark:border-slate-700 px-3 py-1.5 text-sm font-medium text-slate-600 dark:text-slate-300 transition hover:bg-slate-50 dark:hover:bg-slate-700/50 disabled:cursor-not-allowed disabled:opacity-40 dark:bg-slate-800"
          >
            Sebelumnya
          </button>
      ...
      {previewFoto && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center">
          <button
            ...
            aria-label="Tutup"
          />
          <div className="relative z-10 mx-4 max-h-[85vh] max-w-2xl overflow-hidden rounded-2xl bg-white dark:bg-slate-800 shadow-2xl">
